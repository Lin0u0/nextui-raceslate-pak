...
bool rs_reference_load(const char *path, RsReferenceCatalog *out){
    FILE *f; char line[1024]; if(!path||!out)return false; memset(out,0,sizeof(*out)); f=fopen(path,"r"); if(!f)return false;
    if(!fgets(line,sizeof(line),f)){fclose(f);return false;}
    while(out->count<RS_MAX_REFERENCES&&fgets(line,sizeof(line),f)){
        RsCircuitReference *r=&out->circuits[out->count]; char *fields[13],*p,*save=NULL; int i=0;
        p=strtok_r(line,"\t\r\n",&save);
        while(i<13&&p){fields[i++]=p;p=strtok_r(NULL,"\t\r\n",&save);}
        if(i<13)continue;
        ...
        snprintf(r->recent_winners,sizeof(r->recent_winners),"%s",fields[9]);snprintf(r->recent_poles,sizeof(r->recent_poles),"%s",fields[10]);snprintf(r->most_wins,sizeof(r->most_wins),"%s",fields[11]);snprintf(r->most_poles,sizeof(r->most_poles),"%s",fields[12]); out->count++;
    }
