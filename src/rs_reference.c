...
bool rs_reference_load(const char *path, RsReferenceCatalog *out){
    ...
    while(out->count<RS_MAX_REFERENCES&&fgets(line,sizeof(line),f)){
        RsCircuitReference *r=&out->circuits[out->count]; char *fields[10],*p,*save=NULL; int i=0;
        p=strtok_r(line,"\t\r\n",&save);
        while(i<10&&p){fields[i++]=p;p=strtok_r(NULL,"\t\r\n",&save);}
        if(i<10)continue;
        ...
        snprintf(r->recent_winners,sizeof(r->recent_winners),"%s",fields[9]); out->count++;
    }
