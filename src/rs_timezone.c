...
static int64_t transition_utc(int year,int month){struct tm value={0};value.tm_year=year-1900;value.tm_mon=month-1;value.tm_mday=sunday_in_month(year,month,1)-1;value.tm_hour=16;return (int64_t)timegm(&value);}
static int melbourne_dst(int64_t utc,struct tm value){int year=value.tm_year+1900;return utc<transition_utc(year,4)||utc>=transition_utc(year,10);}

int rs_track_utc_offset(const char *id, int64_t utc) {
    struct tm value=utc_parts(utc);
    int europe_dst=european_dst(value);
    if (!id) return 0;
    if (!strcmp(id, "albert_park")) return (melbourne_dst(utc,value) ? 11 : 10) * 3600;
