...
static int month_at(int64_t utc) {
...
int rs_track_utc_offset(const char *id, int64_t utc) {
    int month = month_at(utc);
    int europe_dst = month >= 4 && month <= 10;
    int us_dst = month >= 4 && month <= 10;
    if (!id) return 0;
    ...
    if (!strcmp(id, "miami") || !strcmp(id, "villeneuve")) return (us_dst ? -4 : -5) * 3600;
    if (!strcmp(id, "americas")) return (us_dst ? -5 : -6) * 3600;
    if (!strcmp(id, "vegas")) return (us_dst ? -7 : -8) * 3600;
