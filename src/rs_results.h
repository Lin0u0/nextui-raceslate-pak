...
#define RS_MAX_CLASSIFICATION_ENTRIES 30
typedef enum { RS_RESULT_QUALIFYING, RS_RESULT_SPRINT, RS_RESULT_RACE } RsResultKind;
typedef struct { int position; int grid; int laps; double points; char driver_id[48]; char driver_code[8]; char driver_name[96]; char constructor_name[64]; char status[64]; char time[32]; char q1[24],q2[24],q3[24]; bool fastest_lap; } RsClassificationEntry;
